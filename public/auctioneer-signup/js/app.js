var updateStripePlan = function (plan)
{
    ...
    $('input[name=PlanAmount]')
        .val(plan.amountDecimal);
    console.log(
    {
            ...
            .slider(
            {
                ...
                slide: function (event, ui)
                {
                    ...
                    $(selectors)
                        ...
                        {
                            if ($('#slider1')
                                .val(ui.value) != $('#slider2')
                                .val(ui.value))
                            {
                                $('#slider2')
                                    .slider("option", "value", bidders);
                            }
                        });
                    $('input[name=bidders]')
                        .val($("#bidders")
...
$('body')
    .on('input', 'select[name=stripe_plan]', function ()
    {
...
var oldHouseName = "";
$("body")
    .on("focus", "[name=auction_house_name]", function ()
    {
        oldHouseName = slug($(this)
            .val());
...
$("body")
    .on("keyup", "[name=auction_house_name]", function ()
    {
        if (oldHouseName == $('[name=first_domain_level]')
            .val())
        {
            oldHouseName = slug($(this)
                .val());
...
$("body")
    .on("keyup blur", "[name=first_domain_level]", function ()
    {
...
{
    email_available ? '' : bootbox.alert(
        'The email you are using is already in use by another auction house. If you would like to create an additional house, please email support at <a href="mailto:help@sharpauctionengine.com" target="_top">help@sharpauctionengine.com </a>.'
    );
    ...
    is_available ? '' : bootbox.alert(
        'The sub-domain you are requesting is already in use. Please select a new domain or email support at <a href="mailto:help@sharpauctionengine.com" target="_top"> help@sharpauctionengine.com </a> for additional assistance.'
    );
});
var isDomainTakenAjax = $.debounce(350, function (is_email, $input, $fg, domain, email)
{
    if (minimumDomainLength >= firstDomainLevel(domain)
        .length)
    {
    ...
    else
    {
        $.ajax(
        {
            ...
            complete: function (xhr, textStatus)
            {
                ...
                var is_available = xhr.status === 200 ? json.is_available.domain || false :
                    'Ap_not_available';
                updateHouseAvailableDOM($input, $fg, json.domain, is_available);
                // Email response from node
                if (xhr.status === 200)
                {
                    var fg = $('input[name=email]')
                        .parents('.form-group')
                        .first();
                    $(' #messagebag ')
                        .remove();
                ...
                is_available ? '' : alertTriggersOnDuplicates(is_available, json.domain, json.is_available
                    .email);
...
$("body")
    .on("input", "[name=first_domain_level],[name=email]", function ()
    {
        var $input = $(this);
        var domain = parseInstanceSubDomain($('[name=first_domain_level]')
            .val() || '');
        ...
        $('[name=main_domain]')
            .val(domain);
        isDomainTakenAjax(is_email, $input, $input.parents('.form-group')
            .first(), domain, email);
    });
...
$('body')
    .on('keyup', 'form [data-is-sae-card-exp=1]', denormalizeCardExp);
...
jQuery(function ($)
{
    ...
    $('.registration-form .btn-stripe')
        .on('click', function (event)
        {
            // console.log(this);
            if ($.trim($('input[data-stripe="number"]')
                    .val()))
            {
                var vResult = $('input[data-stripe="number"]')
                    .validateCreditCard();
                console.log(vResult);
                if (!vResult.valid)
                {
            ...
            $form.find('button,input[type=button]')
                .prop('disabled', true);
    ...
    $.ajax(
    {
        ...
        error: function (xhr, textStatus, errorThrown)
        {
            ...
            if (xhr.status == 400)
            {
                ...
                {
                    bootbox.alert(
                        'Your email address already in use! Please try again then contact support.'
                    );
                    $('#email')
                ...
                if (xhr.responseJSON.errors)
                {
                    ...
                    alerts.sprinkle('form:first');
                    bootbox.alert(
                        'There are validation errors. Please click on previous button to review errors'
                    );
                    return;
                }
            }
