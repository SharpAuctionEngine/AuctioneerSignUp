...
var subscribeToStripe = function (stripePlan, userRequestRaw, stripeAPI, cacheRedis) {
    return new Promise(function (resolve, reject) {
        stripePlan.then(function (plan) {
            ...
            var filterMetaDataForStripe=[];
            filterMetaDataForStripe= underscore.omit(userRequestRaw, 'password','repeat_password','stripeToken');
            ...
            cacheRedis.get(cacheKey, function (err, data) {
                if (err || data !== null) {
                    ...
                    stripeAPI.customers.create({
                        ...
                        plan: plan.id,
                        metadata: mapMetaDataForStripe(filterMetaDataForStripe)
